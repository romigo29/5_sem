...
)
{
    ...
    int opt = 1;
    setsockopt(sS, SOL_SOCKET, SO_BROADCAST,
        (char*)&opt, sizeof(opt));
    ...
    sendto(sS, call, (int)strlen(call) + 1, 0,
        (sockaddr*)&all, sizeof(all));

    char buf[64]{};
    *flen = sizeof(SOCKADDR_IN);

    int res = recvfrom(sS, buf, sizeof(buf), 0, from, flen);
    if (res == SOCKET_ERROR)
    {
        closesocket(sS);
        return false;
    ...
    if (strcmp(buf, "connection allowed") == 0)
    {
        closesocket(sS);
        return true;
    }
...
int main()
{
    ...
    try
    {
        ...
        int choice;
        cout << "Choose server search method:\n";
        cout << "1 - Broadcast\n";
        cout << "2 - DNS name\n";
        cout << "Your choice: ";
        cin >> choice;
        ...
        if (choice == 1)
        {
            char name[30] = "ConcurrentServer";

            if (!GetServer(name, (short)2000, (sockaddr*)&serv, &lserv))
                throw "Broadcast search failed";
        ...
        else if (choice == 2)
        {
            ...
            serv.sin_addr = *(in_addr*)host->h_addr_list[0];
        ...
        {
            throw "Invalid search method selected";
        }
        ...
        string request = "Random"; // или "Time"

        if (send(clientSocket, request.c_str(),
            (int)request.length() + 1, 0) == SOCKET_ERROR)
            throw SetErrorMsgText("send: ", WSAGetLastError());

        while (true)
        {
            int result = recv(clientSocket, buffer, BUFFER_SIZE - 1, 0);
