	...
	for (double fi = 0; fi < 360.0; fi += df)
		for (double q = 0; q <= 180.0; q += dq)	
		{
			...
			if (cos_RN > 0) // Если точка видима наблюдателю
			{
				...
				VCart(1) = PV(1);   // Yv
				VCart(2) = 1;
				VCart = VCart - V0; // Xv-xL, Yv-yL, 1  Преобразование в ОСК
				VCart = MW * VCart;   // Преобразование в оконные координаты
